...
# chunks; after each chunk, evaluate 30 gem_grab games. Stop early when
# winrate reaches >= 50% (15 wins). Logs to runs/student_vision/chunked.log
set -e
cd "$(dirname "$0")"
PY=.venv/Scripts/python
DATA=runs/student_vision/bc_data_merged.npz
ACTS=runs/student_vision/bc_acts_merged.npz
CKPT=runs/student_vision/omni2_ep47.pt
EPOCH0=47
...
for round in 2 3 4 5 6; do
    ...
    echo "$RES" | tee -a "$LOG"
    W=$(echo "$RES" | sed -n 's/.* W\([0-9][0-9]*\) .*/\1/p')
    if [ -n "$W" ] && [ "$W" -ge 15 ]; then
